...
contract MultiBetERCExp {
    ...
    constructor(address _tokenAddress) {
        ...
        hyblockToken = IERC20(_tokenAddress);
    ...
    function placeBet(uint256 betId, string memory _option, uint256 amount) public betExists(betId) {
        Bet storage bet = bets[betId];
        require(!bet.isResolved, "Bet has been resolved");
        require(amount > 0, "Bet amount must be greater than zero");

        require(hyblockToken.transferFrom(msg.sender, address(this), amount), "Token transfer failed");

        uint256 optionIndex = _findOptionIndex(bet, _option);
        
        bet.options[optionIndex].totalAmount += amount;
        bet.totalAmount += amount;
        bet.userOptionBetAmount[msg.sender][optionIndex] += amount;
        ...
        emit BetPlaced(betId, msg.sender, amount, _option);
    }
        ...
        if (totalWinnerBetAmount > 0) {
            ...
            for (uint256 i = 0; i < bet.bettors.length; i++) {
                ...
                if (userBet > 0) {
                    ...
                    require(hyblockToken.transfer(user, reward), "Failed to send Ether");
